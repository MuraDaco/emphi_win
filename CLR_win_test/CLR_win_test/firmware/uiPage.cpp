...
// è stato introdotto per poter discriminare la situazione di 'nessun elemento selezionato'
...
void uiPageEventButtonsUp      (void)
{
   ...
   else
   {
      ...
      if(PageListElementId == PtrPageIstanzaElemento->list_element_number)
      {
         ...
         // 1.2.2.2. Azzera l'id della lista, questo siginifica che non sarà selezionato nessun elemento
         PageListElementId = 0;
         return;
      }
      else
...
void uiPageEventButtonsDown    (void)
{
   ...
   else
   {
      ...
      if(PageListElementId == 0)
      {
         ...
         // N.B.: il puntatore alla lista degli elementi verrà impostata al prossimo evento
         return;
      }
      else
      {
...
{
   // controlla se qualche elemento è stato selezionato
   if(PageListElementId > 0)
   {
...
_ROM EventTableDef uiPageEvent[NUMBER_EVENT_BUTTONS] = {
    uiPageEventNull              //    EVENT_BUTTONS_RIGHT     0
   ...
   ,uiPageEventNull              //    EVENT_BUTTONS_LEFT      4
